...
long genPrimeSophieGermain(long min, long max, int *cpt) {
  /// \brief fournit un nombre premier de Sophie Germain vérifié avec le test de
  /// rabin \param[in] min et max \param[out] cpt : le nombre d'essais pour
  /// trouver le nombre \returns un nombre premier p min≤p≤max && p = 2*q+1 avec
  /// q premier
  long num;
  *cpt = 1;
  int a = 2;
...
long seek_generator(long start, long p) {
  /// \brief recherche d'un générateur du groupe (corps) cyclique Z/pZ avec p
  /// premier
  long q =
      (p - 1) /
      2; /// \note p est un premier de Sophie Germain ie p =2q+1 avec q premier

  /// \note si p = \prod(i=1,k,{p_i}^{n_i}) alors g est un générateur si \forall
  /// i 1..k, g^p_i != 1 (mod p) comme p = 2q+1 et q premier, k = 2 et p_i = 2
  /// et q. il suffit que g^2 et g^q soient tous les deux différents de 1 (mod
  /// p)
  while ((puissance_mod_n(start, 2, p) == 1 ||
          puissance_mod_n(start, q, p) == 1) &&
         start < p - 1) {
    start++;
  }
  if (start == p - 1)
...
/// \brief calcule un nombre premier p de Sophie Germain et un générateur du
/// groupe p/Zp. appelle le simulateur d'échange de clef partagée. \returns la
/// clef partagée

// int cpt;
// long premier = genPrimeSophieGermain(min, max, &cpt);
// fprintf(outfp, "premier = %ld # nombre premier de Sophie Germain\n",
// premier); long generateur = seek_generator(3, premier); // exemple 100 long
// ordre =
//    puissance_mod_n(generateur, premier - 1,
// premier); // generateur^{premier -1} (mod premier)
